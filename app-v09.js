...
(() => {
  ...
  async function exportXlsx() {
    if (!window.XLSX) {
      toast("Excel-Bibliothek ist noch nicht geladen. Bitte Internet prüfen und erneut versuchen.");
      return;
    }

    const products = (await getAll())
      .map(p => ({
        product: p.name || p.barcode,
    ...
    const data = [
      ["SPS Lagerübersicht", "", ""],
      ["Exportdatum", new Date().toLocaleString("de-CH"), ""],
      ["Produkte", products.length, ""],
      ["Gesamtbestand", totalStock, ""],
      [],
      ["Produkt", "Bestand", "Letzte Änderung"],
      ...products.map(p => [
        p.product,
        p.stock,
    ...
    worksheet["!autofilter"] = {
      ref: `A6:C${Math.max(6, products.length + 6)}`
    };
    ...
    for (let row = 7; row <= products.length + 6; row++) {
      const stockCell = worksheet[`B${row}`];
      if (stockCell) stockCell.z = "0";

      const dateCell = worksheet[`C${row}`];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Lagerübersicht");
  ...
  function bindEvents() {
    ...
    $("xlsBtn").onclick = () => exportXlsx().catch(err=>{console.error(err);toast("Excel-Export fehlgeschlagen.");});
