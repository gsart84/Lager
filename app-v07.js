(()=>{"use strict";
const DB="sps-lager-db",STORE="articles";let db,scanner=null,current=null,pendingCode="",ocrStream=null,ocrCandidate="",ocrZoom=2.4;
...
async function startOcr(){
  ...
  try{
    ...
    $("ocrVideo").srcObject=ocrStream;$("ocrVideo").style.transform=`scale(${ocrZoom})`;
    await $("ocrVideo").play();
...
function normalizeX20(text){
  ...
  raw=raw
    .replace(/\bX2O\b/g,"X20")
    .replace(/\bXZO\b/g,"X20")
    .replace(/\bX2\s*0\b/g,"X20");

  const compact=raw.replace(/[\s\-]+/g,"");
  const strict=compact.match(/X20[A-Z]{1,4}[0-9]{2,6}/g)||[];
  if(strict.length) return strict.sort((a,b)=>b.length-a.length)[0];

  const loose=compact.match(/X20[A-Z0-9]{4,12}/g)||[];
  return loose.sort((a,b)=>b.length-a.length)[0]||"";
}
async function captureOcr(){
  ...
  btn.classList.add("ocr-busy");
  btn.textContent="X20-Zeile wird gelesen …";
  $("ocrStatus").textContent="Bild wird vergrössert, geschärft und geprüft …";

  try{
    const vw=video.videoWidth, vh=video.videoHeight;
    const sx=Math.round(vw*.19);
    const sy=Math.round(vh*.415);
    const sw=Math.round(vw*.62);
    const sh=Math.round(vh*.14);
    ...
    async function recognizePass(mode){
      ...
      const factor=mode===1?.82:mode===2?.68:.94;
      const threshold=Math.max(55,Math.min(205,avg*factor));

      for(let i=0;i<d.length;i+=4){
        const g=d[i]*.299+d[i+1]*.587+d[i+2]*.114;
        let v;
        if(mode===3) v=Math.max(0,Math.min(255,(g-avg)*2.1+128));
      ...
      const result=await Tesseract.recognize(canvas,"eng",{
        logger:m=>{
          if(m.status==="recognizing text"){
            $("ocrStatus").textContent=`OCR-Prüfung ${mode}/3 · ${Math.round((m.progress||0)*100)} %`;
          }
        },
        tessedit_char_whitelist:"X20ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789- ",
        preserve_interword_spaces:"1"
      });
      const raw=result?.data?.text||"";
      return {raw,code:normalizeX20(raw)};
    }

    let best={raw:"",code:""};
    for(const mode of [1,2,3]){
      const attempt=await recognizePass(mode);
      if(attempt.code.length>best.code.length) best=attempt;
      if(/^X20[A-Z]{1,4}[0-9]{2,6}$/.test(attempt.code)){best=attempt;break}
    }

    if(!best.code){
      $("ocrStatus").textContent="Kein eindeutiger X20-Code erkannt. Typenzeile grösser und gerader ausrichten.";
      toast("Kein X20-Code gefunden.");
      return;
    }

    ocrCandidate=best.code;
    $("ocrResultCode").textContent=best.code;
    $("ocrRawText").textContent=`OCR-Rohtext: ${best.raw.replace(/\s+/g," ").trim().slice(0,120)||"–"}`;
    await stopOcr();
    show("ocrResult");
  }catch(e){
    console.error(e);
    ...
    btn.classList.remove("ocr-busy");
    btn.textContent="X20-Zeile lesen";
  }
}
